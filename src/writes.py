...
def write_xmp(d: dict[str, any], filename: str, simulating: bool):
    if simulating:
        directory_path = os.path.join(XMP_PATH, "|".join(filename.split("|")[:-1]))
        if not os.path.exists(directory_path):
            os.mkdir(directory_path)
        with open(
            os.path.join(directory_path, f"{filename}.xmp"), "w"
        ) as xmp_output_file:
...
def write_yaml(d: dict[str, any], filename: str, simulating: bool):
    if simulating:
        directory_path = os.path.join(YAML_PATH, "|".join(filename.split("|")[:-1]))
        if not os.path.exists(directory_path):
            os.mkdir(directory_path)
        with open(
            os.path.join(directory_path, f"{filename}.yaml"), "w"
        ) as yaml_output_file:
